from ..core.finance import propose_financing
...
def run_finance_wizard(local_price: float, equip_cost: float):
    ...
    print("• Prêt BPI (max)                : 20 000 € (accordé d'office, ajusté selon besoin)")
    ...
    print(f"Frais d'installation inclus         : {plan['invest_total_with_fees']:.0f} €")
    print(f"Apport (admin)                      : {plan['apport']:.0f} €")
    print(f"Crédit Banque (fixe)                : {plan['bank_amount']:.0f} € -> {plan['bank_monthly']:.0f} €/mois")
    print(f"Crédit BPI (<=20k)                  : {plan['bpi_amount']:.0f} € -> {plan['bpi_monthly']:.0f} €/mois")
