...
from FoodOPS_V1.data.menus_presets_simple import get_default_menus_simple
...
def _print_opening_balance(restaurant: Restaurant):
    ...
    print("ACTIF")
    print(
        f"  🏭 Immobilisations (215)        : {_fmtformat_currency_eur(a['Immobilisations (215)'])}"
    )
    print(
        f"  (–) Amort. cumulés (2815)      : {_fmtformat_currency_eur(a['Amortissements cumulés (2815)'])}"
    )
    print(
        f"  =  Immobilisations nettes      : {_fmtformat_currency_eur(a['Immobilisations nettes'])}"
    )
    print(
        f"  💶 Trésorerie (512)             : {_fmtformat_currency_eur(a['Trésorerie (512)'])}"
    )
    print(
        f"  👉 TOTAL ACTIF                  : {_fmtformat_currency_eur(a['Total Actif'])}"
    )

    print("\nPASSIF")
    print(
        f"  🧱 Capitaux propres (101)       : {_fmtformat_currency_eur(p['Capitaux propres (101)'])}"
    )
    print(
        f"  🏦 Emprunts (164)               : {_fmtformat_currency_eur(p['Emprunts (164)'])}"
    )
    print(
        f"  👉 TOTAL PASSIF                 : {_fmtformat_currency_eur(p['Total Passif'])}"
    )
    print("═" * 52)
...
def create_restaurants():
    ...
    for i in range(nb_joueurs):
        ...
        local = CATALOG_LOCALS[type_resto][0]

        # Équipement par défaut selon type (tu pourras affiner)
        equip_default = (
            80_000.0
            if type_resto == "FAST_FOOD"
            else (120_000.0 if type_resto == "BISTRO" else 180_000.0)
        )
        ...
        print(f"\n💼  {r.name} — {r.type.value}")
        print(
            f"📍 Local: {local.nom}  |  Capacité: {local.capacite_clients} couverts/jour  |  Loyer: {_fmtformat_currency_eur(local.loyer)}/mois"
        )
        print(f"🧰 Équipement initial : {_fmtformat_currency_eur(equip_default)}")
        print(
            f"🏦 Banque : {_fmtformat_currency_eur(plan.bank_loan)}  → Mensualité ~ {_fmtformat_currency_eur(plan.bank_monthly)}"
        )
        print(
            f"🏛️ BPI   : {_fmtformat_currency_eur(plan.bpi_loan)}   → Mensualité ~ {_fmtformat_currency_eur(plan.bpi_monthly)}"
        )
        print(
            f"🧾 Frais de dossier (3%) : {_fmtformat_currency_eur(plan.frais_dossier)}"
        )
        print(
            f"💶 Trésorerie de départ  : {_fmtformat_currency_eur(plan.cash_initial)}"
        )
