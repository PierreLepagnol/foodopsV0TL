...
from FoodOPS_V1.core.game_types import TurnResult
from FoodOPS_V1.rules.costing import compute_average_unit_cogs
...
def compute_turn_result(r, tour, clients_attrib, clients_servis, prix_median):
    ...
    # Charges RH
    rh_cost = (
        sum(emp.salaire_total for emp in getattr(r, "employes", []))
        if hasattr(r, "employes")
        else 0.0
    )
